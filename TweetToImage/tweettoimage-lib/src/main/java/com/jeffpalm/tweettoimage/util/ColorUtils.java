...
// From: http://www.java2s
// .com/Code/Android/2D-Graphics/Returnsthecomplimentaryoppositecolor.htm
...
public final class ColorUtils {
   ...
   * @return int RGB of compliment color
   */
  public static int getComplimentGrayColor(int color) {
    // get existing colors
    int alpha = Color.alpha(color);
    int red = Color.red(color);
    int blue = Color.blue(color);
  ...
  private static String ARGBToHex(int rgbVal) {
    ...
    return String.valueOf(hexReference.charAt((rgbVal - rgbVal % 16) / 16) + "" + hexReference
        .charAt(
        rgbVal % 16));
  }
}
